...
## Overview
The core module is the heart of the SoundTrack library, 
...
## Features
- `Configuration Management:` Centralizes the configuration settings for the entire SoundTrack library,
...
providing developers with the ability to optimize their usage of SoundTrack.

## Usage
To utilize the core module, simply import it at the beginning of your script:

```python
from soundtrack import __core__
```
"""
...
def __checkupdates__(user, repo):
    """
    Checks for updates to the SoundTrack package on GitHub.

    This function compares the current version of SoundTrack, defined in the module,
    ...
    if current_version < update_version:
        print(f"[{fsconsole.foreground.BLUE}Notice{fsconsole.style.RESET_ALL}]: A new release of SoundTrack is available: {fsconsole.foreground.RED}v{__version__}{fsconsole.style.RESET_ALL} -> {fsconsole.foreground.GREEN}{tag_name}{fsconsole.style.RESET_ALL}")
        print(f"[{fsconsole.foreground.BLUE}Notice{fsconsole.style.RESET_ALL}]: To update, run: {fsconsole.foreground.GREEN}pip install --upgrade soundtrack{fsconsole.style.RESET_ALL}")